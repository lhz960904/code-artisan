...
// Pattern B: agent loop runs server-side, tool calls are dispatched to E2B
// microVMs via the SDK. Value class — not @Injectable; lifecycle is owned by
// SandboxPoolService.
...
export const DEFAULT_SANDBOX_LIFETIME_MS = 10 * 60 * 1000;
// Custom E2B template: extends code-interpreter with Bun + preloaded Skills at
// /opt/skills. See `sandbox-template/e2b.Dockerfile`.
const E2B_TEMPLATE_NAME = "code-artisan";
// E2B's default pty timeout is 60s — way too short for an interactive shell or
// dev server. Set to 1h (Hobby max sandbox lifetime); the sandbox's own
// idle-kill timer terminates the PTY long before this fires.
...
export class E2BSandbox implements Sandbox {
  ...
  // Consumed by ShellSessionManagerService to back long-running shell sessions.
  readonly pty = {
  ...
  async glob(pattern: string, path: string): Promise<GlobResult> {
    // Path-prefix patterns (e.g. "src/**/*.ts") are approximated by basename
    // matching — use the bash tool when finer control is needed.
    try {
  ...
  static async create(timeoutMs: number = DEFAULT_SANDBOX_LIFETIME_MS): Promise<E2BSandbox> {
    ...
    // Using files.makeDir (not exec) avoids chicken-and-egg with default cwd.
    try {
