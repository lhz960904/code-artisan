...
describe("ClaudeService", () => {
  it("should return a text response for a simple question", async () => {
    ...
    const response = await claude.chat([{ role: "user", content: "Say hello in exactly one word." }]);
  ...
  it("should return a tool_use response when appropriate", async () => {
    const claude = new ClaudeService();
    const response = await claude.chat([{ role: "user", content: "List the files in /tmp directory." }]);
