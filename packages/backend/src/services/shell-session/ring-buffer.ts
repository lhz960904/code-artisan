...
/** Append-only chunk ring keyed by cumulative byte offset. Total kept
 *  bytes ≤ `capacityBytes`; oldest chunks get dropped when the cap is hit.
 *  Readers pass a `sinceOffset`; anything older than `oldestOffset()` has
 *  been evicted (caller sees `truncated: true`). */
export class RingBuffer {
  ...
  /** Offset of the oldest byte still in the buffer. */
  oldestOffset(): number {
    return this.chunks[0]?.offset ?? this.writeOffset;
  }

  /** Offset just past the newest byte. */
  endOffset(): number {
    return this.writeOffset;
  }

  /** Read bytes starting at `sinceOffset` (or the oldest kept offset if that's
   *  older than what's buffered), returning at most `maxBytes`. */
  readFrom(sinceOffset: number, maxBytes: number): { data: string; nextOffset: number; truncated: boolean } {
  ...
  /** Read up to `maxBytes` from the tail. Useful for attach snapshots. */
