/**
 * E2B Sandbox implementation that conforms to @code-artisan/agent's
 * Sandbox interface. Passed into Agent via `createAgent({ sandbox })`.
 *
 * This is the canonical sandbox implementation for the web (Pattern B)
 * execution model: the agent loop runs server-side, tool calls are
 * dispatched to E2B microVMs over the SDK.
 *
 * NOTE: The legacy `e2b-sandbox.ts` in this directory is a parallel
 * implementation used by backend's own tool runtime (not yet migrated).
 * When that runtime is retired, this file can be renamed to
 * `e2b-sandbox.ts` as the single E2B sandbox implementation.
 */
